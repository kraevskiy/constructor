...
export class LayoutService {
  constructor(
    @InjectModel(LayoutModel)
    private readonly layoutModel: ModelType<LayoutModel>,
  ) {}
  ...
  async findAll(
    dto: FindLayoutsDto,
  ): Promise<DocumentType<LayoutModel>[] | null> {
    ...
    function generateMatch() {
      const newMatch: { [key: string]: string|boolean } = {};
      if (dto.user) {
        newMatch['user'] = dto.user;
      }
      if (dto.public) {
        newMatch['public'] = dto.public;
      }
      return {
        $match: newMatch,
      };
